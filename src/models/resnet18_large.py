...
class ResNet(nn.Module):
    def __init__(
        ...
        # self.conv1 = nn.Conv2d(3, self.inplanes, kernel_size=3, stride=1, padding=1, bias=False) # Activate stride=1 for resolution 32x32 & 64x64
        self.conv1 = nn.Conv2d(
            3, self.inplanes, kernel_size=3, stride=2, padding=1, bias=False
        )  # Activate stride=2 for res 128x128 & 224x224
        self.bn1 = norm_layer(self.inplanes, affine=affine)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(
            kernel_size=3, stride=2, padding=1
        )  # Activate for res 128x128 & 224x224 otherwise deactivate
