...
from data_loader import (
    get_data_loader,
    get_down_up_sampled_Imagenet100_dataloader,
    get_ImageNet100_dataloader,
)
from models.models import get_resnet34_by_resolution
from utils.utils import EarlyStopper


def train(model, device, train_dataset, test_dataset, learning_rate, weight_decay, epochs):
    save_dir = f"weights/resnet34_resolutions/{image_resolution}_wide/{str(learning_rate)}/{str(weight_decay)}"
...
if __name__ == "__main__":
    ...
    model = get_resnet34_by_resolution(
        image_resolution, class_num=class_num, wide=True, pretrained=False
    )
    model.train()
